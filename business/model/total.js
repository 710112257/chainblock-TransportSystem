...
async function login(req) {
    try {
        ...
        await users.find({ 'username': username }, {}, function (err, result) {
        console.log(result);
        if (err) {
            console.log(err);
            ok=1;
        ...
        console.error(`Failed to connction: ${error}`);
        return false;
    }
...
async function putValue(assertId,optContent,ccp) {
    try {
        ...
        // 创建一个用于与peer节点通信的网关对象
        const contract = network.getContract('cc_business');
        // 提交特定的交易 .
        //注意，涉及到修改状态使用submitTransaction
        const TXID=await contract.submitTransaction('putValue', assertId, JSON.stringify(optContent));
        console.log('交易已提交');
        // 断开网关连接
...
async function delDate(assertId,ccp) {
    try {
        // 创建一个用于管理身份的文件夹wallet
        const walletPath = path.join(__dirname, '../wallet/');
        const wallet = new FileSystemWallet(walletPath);
        ...
        const TXID=await contract.submitTransaction('delDate', assertId);
        console.log('交易已提交');
        // 断开网关连接
        await gateway.disconnect();
        return TXID.toString();

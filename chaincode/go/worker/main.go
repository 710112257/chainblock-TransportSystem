...
func (t *SmartContract) Invoke(stub shim.ChaincodeStubInterface) peer.Response{
	...
	if args[0] == "putValue"{
		return invokeOtherChainCode(stub, args)        // 添加信息
	}else if args[0] == "getValue" {
		return invokeOtherChainCode(stub, args)        // 获取
	}else if args[0] == "getHistory" {
		return invokeOtherChainCode(stub, args)  // 根据订单编号查询信息
	}else if args[0] == "upDate" {
		return invokeOtherChainCode(stub, args)
	}
